...
/**
 * ExoPlayer AudioProcessor that uses the Rubberband library for time-stretching.
...
class RubberbandAudioProcessor : BaseAudioProcessor() {
    ...
    companion object {
        private const val TAG = "RubberbandAudioProc"
    ...
    init {
        try {
            RubberbandNative.ensureLoaded()
            nativeLibraryLoaded = true
            Log.i(TAG, "Rubberband native library loaded successfully!")
        } catch (e: Exception) {
            Log.e(TAG, "Failed to load Rubberband native library", e)
    ...
    fun setSpeed(speed: Float) {
        ...
        if (handle != 0L && enabled) {
            ...
            RubberbandNative.setTimeRatio(handle, timeRatio)
            Log.i(TAG, "Applied Rubberband timeRatio: $timeRatio for speed ${this.speed}")
        } else {
    ...
    /**
     * Enable or disable the Rubberband processing.
     * When disabled, audio passes through unchanged.
     */
    fun setEnabled(enabled: Boolean) {
        this.enabled = enabled
        Log.d(TAG, "Rubberband processing ${if (enabled) "enabled" else "disabled"}")
    }
    
    /**
     * Check if Rubberband processing is active.
     * Returns true if enabled and speed != 1.0
     */
    fun isRubberbandActive(): Boolean {
    ...
    override fun onConfigure(inputAudioFormat: AudioFormat): AudioFormat {
        ...
        if (handle != 0L && 
            ...
            RubberbandNative.destroy(handle)
            handle = 0
        }
        
        // Create new Rubberband instance
        if (handle == 0L && enabled) {
            handle = RubberbandNative.create(
            ...
            if (handle != 0L) {
                ...
                Log.e(TAG, "Failed to create Rubberband instance")
                enabled = false
            }
        }
    ...
    override fun queueInput(inputBuffer: ByteBuffer) {
        ...
        if (frameCounter % 100 == 0L) {
            Log.d(TAG, "queueInput: frame=$frameCounter, active=${isRubberbandActive()}, speed=$speed, handle=$handle, bytes=${inputBuffer.remaining()}")
        }
        
        // Pass through if not active or no input
        if (!isRubberbandActive() || !inputBuffer.hasRemaining()) {
        ...
        RubberbandNative.process(handle, inputShortBuffer, sampleFrames)
        
        // Retrieve available output
        val availableFrames = RubberbandNative.available(handle)
        if (availableFrames > 0) {
            ...
            val retrievedFrames = RubberbandNative.retrieve(handle, outputShortBuffer, availableFrames)
    ...
    override fun onQueueEndOfStream() {
        ...
        if (handle != 0L && enabled) {
            ...
            var availableFrames = RubberbandNative.available(handle)
            if (availableFrames > 0) {
                val outputSamples = availableFrames * channels
                if (outputShortBuffer.size < outputSamples) {
                    outputShortBuffer = ShortArray(outputSamples)
    ...
    override fun onFlush() {
        super.onFlush()
        if (handle != 0L) {
            RubberbandNative.reset(handle)
            Log.d(TAG, "Flushed Rubberband state")
        }
    }
    ...
    override fun onReset() {
        ...
        if (handle != 0L) {
            RubberbandNative.destroy(handle)
            handle = 0
            Log.d(TAG, "Reset: destroyed Rubberband instance")
        }
    ...
    /**
     * Get the latency introduced by Rubberband in milliseconds.
     */
    fun getLatencyMs(): Int {
        if (handle == 0L || configuredSampleRate == 0) return 0
        val latencyFrames = RubberbandNative.getLatency(handle)
